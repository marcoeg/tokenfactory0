...
import (
	...
	"github.com/marcog/tokenfactory/testutil/network"
	"github.com/marcog/tokenfactory/x/tokenfactory/client/cli"
)
...
func TestDeleteDenom(t *testing.T) {
	net := network.New(t)
	...
	for _, tc := range []struct {
		...
		t.Run(tc.desc, func(t *testing.T) {
			...
			args = append(args, tc.args...)
			out, err := clitestutil.ExecTestCLICmd(ctx, cli.CmdDeleteDenom(), args)
			if tc.err != nil {
				require.ErrorIs(t, err, tc.err)
			} else {
				require.NoError(t, err)
